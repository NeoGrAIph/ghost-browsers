...
- **Playwright CLI compatibility**: начиная с Playwright 1.55 runner вызывает `playwright launch-server --browser <name>` и транслирует публичное имя `camoufox` в `firefox`, параллельно прокидывая `CAMOUFOX_BINARY`, чтобы использовать Camoufox runtime вместо стандартного Firefox.
...
- 2025-10-03 · gpt-5-codex · Обновлён `app.browser.launch_browser`: Playwright CLI вызывается с `--browser`, `camoufox` транслируется в `firefox`; добавлены unit-тесты `test_browser.py` для проверки команды и окружения.
