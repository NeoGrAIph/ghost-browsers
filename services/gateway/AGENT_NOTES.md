...
## Interfaces
- REST:
  - `POST /sessions` — проксирование создания новой сессии на Runner + регистрация результата.
  - `POST /sessions/register` — регистрация сессии от Runner (service-to-service).
...
- UI принимает `POST /sessions` от оператора, сам выбирает runner, делегирует создание и регистрирует ответ без раскрытия runner наружу.
...
- 2025-10-09 · gpt-5-codex · Добавлен прокси `POST /sessions` в Runner, выделен `POST /sessions/register` для сервисного канала, обновлены тесты.
