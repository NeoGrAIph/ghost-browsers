...
- `SessionRegistry` перед сохранением очищает `Session.vnc` от временных токенов и query `token`/`access_token`, чтобы в памяти оставались только долгоживущие поля; новые токены выдаются на каждую отдачу через роутеры.
...
- Эндпоинт `/sessions/{id}/touch` валидирует временную метку heartbeat: наивные значения отклоняются с HTTP 400, а timezone-aware
  значения конвертируются в UTC перед сохранением в реестр.
...
- Чтобы избежать 503 на первом запросе в `docker compose`, если health-пробы ещё не успели отметиться, `/sessions/commands` при отсутствии здоровых кандидатов повторно запрашивает реестр без фильтра `healthy`, оставляя проверки VNC и слотов. Как только health-чек проходит, выбор снова возвращается к здоровым раннерам.
...
- Discovery/static конфигурации Runner обязаны публиковать capability-флаги (`browser:`/`region:`/`proxy:`); `/runners` сортирует
  `Runner.capabilities` и отдаёт массивы UI для построения composer и фильтров.
...
- `SessionRegistry.touch` принимает только timezone-aware временные метки; ValidationError при обновлении heartbeat логируется и преобразуется в HTTP 400 вместо серверной ошибки.
...
- 2025-10-04 · gpt-5-codex · Реализован fallback выбора раннера в `/sessions/commands`, чтобы первый запуск после `docker compose up` сразу создавал сессию; добавлены unit-тесты и обновлены smoke-проверки.
...
- 2025-11-01 · gpt-5-codex · Защитил `/sessions/{id}/touch` от наивных временных меток и добавил обработку ValidationError в SessionRegistry.touch.
- 2025-10-04 · gpt-5-codex · `/runners` возвращает capability-флаги для UI, добавлен unit-тест и compose-конфиг публикует демонстрационные `browser`/`region`.
