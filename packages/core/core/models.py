...
class SessionProxySettings(BaseModel):
    ...
    def ensure_at_least_one_proxy(self) -> "SessionProxySettings":
        ...
        Returns:
            SessionProxySettings: The validated proxy settings instance.

        Raises:
            ValueError: If all proxy URL fields are ``None``.
        """
...
class SessionVncDetails(BaseModel):
    ...
    def _trim_token(cls, value: str | None) -> str | None:
        """Normalise token values emitted by the gateway.

        Args:
            value: Raw token string or ``None`` when no token is provided.

        Returns:
            str | None: Trimmed token value, or ``None`` if no token is set.

        Raises:
            ValueError: If the token is composed solely of whitespace characters.
        """
    ...
    def check_payload(self) -> "SessionVncDetails":
        """Validate the payload against configuration constraints.

        Returns:
            SessionVncDetails: The validated VNC detail instance.

        Raises:
            ValueError: If both URLs are missing, if the token TTL exceeds
                300 seconds, or when a token lacks its TTL counterpart.
        """
...
class Runner(BaseModel):
    ...
    def _strip_identifier(cls, value: str) -> str:
        """Normalise runner identifiers coming from discovery.

        Args:
            value: Raw identifier supplied by the runner or discovery mechanism.

        Returns:
            str: Trimmed identifier ready for storage and comparison.

        Raises:
            ValueError: If the resulting identifier is empty.
        """
    ...
    def _post_init(self) -> "Runner":
        """Compute derived defaults and enforce field relationships.

        Returns:
            Runner: The validated runner instance.

        Raises:
            ValueError: If the available slots fall outside 0..total_slots,
                if an offline runner is marked healthy, or when heartbeat
                timestamps lack timezone information.
        """
...
class Session(BaseModel):
    ...
    def _trim_runner_id(cls, value: str) -> str:
        """Normalise runner identifiers embedded inside sessions.

        Args:
            value: Runner identifier reported by the emitting service.

        Returns:
            str: Trimmed runner identifier.
    ...
    def _trim_browser(cls, value: str) -> str:
        ...
        Args:
            value: Raw browser string received from the runner.

        Returns:
            str: Trimmed browser name suitable for display.

        Raises:
            ValueError: If the browser field is empty after trimming.
        """
    ...
    def _clean_ws_endpoint(cls, value: str | None) -> str | None:
        """Validate the optional WebSocket endpoint value.

        Args:
            value: Endpoint string or ``None`` if the runner does not expose one.

        Returns:
            str | None: Trimmed endpoint string, or ``None`` when absent.

        Raises:
            ValueError: If a provided endpoint consists solely of whitespace.
        """
    ...
    def _validate_labels(cls, value: dict[str, str]) -> dict[str, str]:
        """Normalise label keys and values to trimmed strings.

        Args:
            value: Mapping of raw label keys to raw values.

        Returns:
            dict[str, str]: Cleaned mapping with trimmed keys and values.

        Raises:
            ValueError: If any label key becomes empty after trimming.
        """
    ...
    def _validate_temporal_relationships(self) -> "Session":
        ...
        Raises:
            ValueError: If any timestamp lacks timezone information or if
                temporal ordering invariants are violated.
        """
    ...
    def updated_at(self) -> datetime:
        """Return a backwards-compatible alias for :attr:`last_seen_at`.

        Returns:
            datetime: Timestamp representing the moment the session was last
...
class SessionEvent(BaseModel):
    ...
    def _validate_event(self) -> "SessionEvent":
        ...
        Raises:
            ValueError: If timestamps are naive, or if the session status does
                not align with the declared event type.
        """
    ...
    def runner_id(self) -> str:
        ...
        Returns:
            str: Runner identifier associated with the session snapshot.
        """
    ...
    def is_terminal(self) -> bool:
        ...
        Returns:
            bool: ``True`` if the session status is :class:`SessionStatus.DEAD`.
        """
