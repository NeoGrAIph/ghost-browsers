...
## Overview
Консоль оператора Ghost Browsers на React + Vite. Обеспечивает авторизацию через Keycloak, отображение списка сессий браузера, просмотр деталей/снимков, закрепление VNC-превью, wallboard и управление (создание/удаление, прокси).
...
- UI-паттерн split-view: табличное представление сессий + инспектор справа.
- Принята компоновка à la Camofleet: таблица, инспектор с быстрыми действиями, панель закреплённых превью и отдельная wallboard-вкладка.
- Фирменный знак в топбаре теперь рисуется собственным SVG-привидением, чтобы закрепить дружественный визуальный язык без зависимости от emoji-рендеринга.
- Состояние закреплённых превью фильтруется без лишних перерисовок: функция `setPinnedIds` возвращает исходный список, если состав не изменился, чтобы избежать бесконечных циклов эффектов при одинаковых данных.
...
- [ ] Реализовать управление TTL (touch) и быстрый рестарт прямо из инспектора после появления API.
...
- 2025-11-21 · gpt-5-codex · Обновлён топбар с SVG-привидением, исправлена логика закреплённых превью (без лишних setState) и переработан `SessionActions` для корректной работы мутаций без замыкания на устаревшую сессию.
